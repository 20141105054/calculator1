...
//void CcalculatorDlg::OnNextMenu(UINT nKey,LPMDINEXTMENU lpMdiNextMenu)
//{
//	// TODO: 在此添加消息处理程序代码和/或调用默认值
//
//	CDialogEx::OnNextMenu(nKey, lpMdiNextMenu);
//}
...
void CcalculatorDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	CPen yuandian(PS_SOLID,1,RGB(0,0,0));
	oldpen=pDC->SelectObject(&yuandian);
	pDC->Ellipse(-1,-1,1,1);
	...
	oldpen=pDC->SelectObject(&groundpen);//分针
	x=L2*sin(minag);
	y=-L2*cos(minag);
	pDC->MoveTo(0,0);
	pDC->LineTo(x,y);
	...
	oldpen=pDC->SelectObject(&groundpen);//时针
	x=L3*sin(hag);
	y=-L3*cos(hag);
	pDC->MoveTo(0,0);
	pDC->LineTo(x,y);
